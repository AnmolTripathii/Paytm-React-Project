...
const Dashboard = () => {
    ...
    const fetchTransactionInfo = async () => {
        try {
            ...
            const response = await axios.get("https://paytm-react-project.onrender.com/api/v1/account/dashboard-transaction-info", {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
    ...
    const fetchWeeklyTransactions = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get('https://paytm-react-project.onrender.com/api/v1/account/transaction-weekly-data', {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
